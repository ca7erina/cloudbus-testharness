...
import tests.testObject.TestLinkCase;
import tests.testObject.TestStep;
...
public class ReportUtil {
	...
	public static void getReport() {
		try {
			FileUtils.copyFile(new File(testResultSamplePath), new File(
					testResultPath));
	...
	public static void updateValuePrepare(int run, int fail, long duration) {
		...
		String testEnvironment = EnvDetector.testEvn;
		...
		if (methodName.split("\\$").length > 1) {
			String caseid = methodName.split("\\$")[1];
				...
				if (Integer.parseInt(id[1]) >= 1) {
					...
					logger.info("there is no info about this step");
				}
				}
			// TestLinkCase c=
			// TestCaseLoader.getCaseByid(Integer.parseInt(id[0]));
			...
			TestLinkCase c;
			try {
				c = TestCaseLoader.getCaseByid(Integer.parseInt(id[0]));
				if (c != null) {
					insertReportLine(String.valueOf(getIndex()),
							"CB" + c.getExternalid(), c.getTestSuitName(),
							c.getName(), ifPass == true ? "pass" : "fail",
							methodName, "none");
				} else {
					logger.info(" testlinkcase =null, cannot find this case, no case no report:(");
				}
			} catch (Exception e) {
				logger.info(" cannot get  testcase info");
				e.printStackTrace();
				return;
			}
		} else {
			logger.info(" ignore caseid, no record in testlink, will not report ");
		}
	}

	public static void reportToDB(String methodName, boolean ifPass) {
		String caseid = methodName.split("\\$")[1];
		String id[] = caseid.split("_");
		TestStep step = new TestStep();
		if (!id[1].equals("0")) {
